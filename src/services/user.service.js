...
const userService = {
    ...
    const verificationCode = crypto.randomInt(10000000, 99999999).toString(); //eight characters randomly generated
    ...
    const user = await User.create({
      ...userData, //spread operator to get all user data from request body
    ...
    process.nextTick(() => {
      mailService.sendWelcomeMail(user).catch(console.error); //to catch email sending error
    });
  ...
  refreshAccessToken: async (refreshToken, next) => {
   ...
   verifyUserAccount: async (data, next) => {
    ...
    //check if verificationToken saved in db is same as the one received from the
